...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	do {
		rcvBuf[i] = tmpBuf[0];
		HAL_UART_Receive(&huart3, tmpBuf, 1, 3000);
		i++;
	}
	while (tmpBuf[0] != '\r');
